    ...
    if ft == 3 && beta < 1000
        ...
        if M_ndPhys>mndobj
            ...
            if loop>1 
                ...
                if M_ndTilde_old-M_ndTilde<0.2 %% DETECT CONGEsTION 
                    s=s+1;
                end
            end
            if s>3 %% SWITCH TO PHASE 2
                beta_i = logspace(0,3,100); %% LOG-SPACED BETA SAMPLES
                ...
                M_ndTarget=avg_decrease_ratio*M_ndPhys; %% TARGET GRAY LEVEL INDICATOR
                beta=min(betamax,interp1(M_ndPhys_Predict,beta_i,M_ndTarget)); %% PHASE 2 UPDATE
            end
        end
